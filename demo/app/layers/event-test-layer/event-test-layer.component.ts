...
export class EventTestLayerComponent implements OnInit {
	...
	constructor(private eventManager: MapSelectionService,
	            public  plonterService: PlonterService,private mapSelectionService: MapSelectionService,
	...
	ngOnInit(): void {
		...
		this.mapSelectionService.select({event: CesiumEvent.LEFT_CLICK,
            pick: PickOptions.PICK_FIRST}).subscribe((pos) => {
    ...
    multiSelect() {
        ...
        this.mapSelectionService.select(inputConf).map((result) => result.entities).subscribe((entities) => {
